...
public class AccountList extends GuiListExtended {
    ...
    public class AccountEntry implements GuiListExtended.IGuiListEntry {
        ...
        public void drawEntry(int i, int x, int y, int w, int h, int mx, int my, boolean hover, float delta) {
            ...
            // Show token expiry for TokenAccounts
            if (account instanceof TokenAccount) {
                TokenAccount tokenAcc = (TokenAccount) account;
                String timeRemaining = tokenAcc.getTimeRemaining();
                // Use isLikelyExpired() since tokens don't have exact expiry
                int timeColor = tokenAcc.isLikelyExpired() ? 0xFF0000 : 0xFFFF00;
                mc.fontRenderer.drawString("(" + timeRemaining + ")",
                        x + 10 + mc.fontRenderer.getStringWidth(account.name()) + 5, y + 1, timeColor);
            }
